    ...
    for scene_dir in model_paths:
        try:
            ...
            for method in os.listdir(test_dir):
                ...
                image_names = readImages(renders_dir, gt_dir)
                ...
                for fname in tqdm(image_names, desc="Metric evaluation progress"):
                    with Image.open(renders_dir / fname) as render_image, Image.open(gt_dir / fname) as gt_image:
                        render = tf.to_tensor(render_image).unsqueeze(0)[:, :3, :, :].cuda()
                        gt = tf.to_tensor(gt_image).unsqueeze(0)[:, :3, :, :].cuda()
                    ssims.append(ssim(render, gt))
                    psnrs.append(psnr(render, gt))
                    lpipss.append(lpips(render, gt, net_type='vgg'))
                    ms_ssims.append(ms_ssim(render, gt, data_range=1, size_average=True))
                    lpipsa.append(lpips(render, gt, net_type='alex'))
