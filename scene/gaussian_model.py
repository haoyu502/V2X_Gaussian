...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, time_line: int):
        ...
        max_points = getattr(self, "max_gaussians", 0)
        if max_points > 0 and pcd_points.shape[0] > max_points:
            # Deterministic uniform sampling keeps the initialization below the
            # hard memory budget before any CUDA/Adam allocation occurs.
            keep = np.linspace(0, pcd_points.shape[0] - 1, max_points, dtype=np.int64)
            pcd_points = pcd_points[keep]
            pcd_colors = pcd_colors[keep]
            print(f"Initial point cloud limited to {max_points:,} points")
        # breakpoint()
    ...
    def training_setup(self, training_args):
        ...
        self.max_gaussians = getattr(training_args, "max_gaussians", 0)
        self.max_densify_points = getattr(training_args, "max_densify_points", 0)
    ...
    @staticmethod
    def _limit_mask(mask, scores, limit):
        if limit <= 0 or int(mask.sum()) <= limit:
            return mask
        indices = torch.nonzero(mask, as_tuple=False).squeeze(1)
        chosen = indices[torch.topk(scores[indices], k=int(limit), sorted=False).indices]
        limited = torch.zeros_like(mask)
        limited[chosen] = True
        return limited

    def densify_and_split(self, grads, grad_threshold, scene_extent, box3ds, N=2, max_new_points=0):
        ...
        source_limit = max_new_points // N if max_new_points > 0 else 0
        selected_pts_mask = self._limit_mask(selected_pts_mask, padded_grad, source_limit)
    ...
    def densify(self, max_grad, min_opacity, extent, max_screen_size, density_threshold, displacement_scale,
        ...
        if self.max_gaussians > 0:
            remaining = max(0, self.max_gaussians - self.get_xyz.shape[0])
            if remaining == 0:
                return
        budget = self.max_densify_points
        if remaining > 0:
        ...
        if budget > 0:
            ...
            if budget == 0:
                return
        self.densify_and_split(grads, max_grad, extent, box3ds, max_new_points=budget)
