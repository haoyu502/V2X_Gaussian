...
def fetchPly(path):
    ...
    return BasicPointCloud(points=positions, colors=SH2RGB(shs), normals=normals)
...
def readV2XGOTRinfos(datadir, args=None):

    ply_path = os.path.join(datadir, "lidar_downsampled.ply")

    from scene.V2XGOTR_dataset import V2XGOTR_Dataset
    resolution = getattr(args, "resolution", 1)
    downsample = float(resolution) if resolution in (1, 2, 4, 8) else 1.0
    train_dataset = V2XGOTR_Dataset(
    datadir,
    "train",
    downsample,
    time_scale=1,
    ...
    eval_index=0,
    keyframe_count=getattr(args, "keyframe_count", 0),
    keyframe_min_gap=getattr(args, "keyframe_min_gap", 1),
        )
    test_dataset = V2XGOTR_Dataset(
    datadir,
    "test",
    downsample,
    time_scale=1,
